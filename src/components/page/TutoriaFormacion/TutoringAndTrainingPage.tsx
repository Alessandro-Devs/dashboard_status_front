...
function AccessView({ onNext }: {
    ...
    const access = { ...data.accesos, docentes: formatQuantity(data.accesos.docentes), estudiantes: formatQuantity(data.accesos.estudiantes) };
...
function ModelingView({ onBack, onNext }: {
    ...
    return <section className="mt-6"><ModelamientosSection model={model}/><Navigation onBack={onBack} back="Accesos" onNext={onNext} next="Ver Diagnósticos"/></section>;
}
type ModelingData = typeof data.modelamientos;
function ModelamientosSection({ model: sourceModel }: {
    model: ModelingData;
}) {
    const model = { ...sourceModel, meta: { ...sourceModel.meta, total: formatQuantity(sourceModel.meta.total), realizados: formatQuantity(sourceModel.meta.realizados) } };
    const distributionCards = sortDescendingByNumber([
        { title: "Clase regular + remediación", value: model.claseRegularRemediacion.realizados, percentage: Number(model.claseRegularRemediacion.porcentaje) || 0, color: "green" as const },
...
function TutoriaVirtual() {
    const tutoringData = (data.tutoriaVirtual as TutoringGroup[]).map((group, index) => ({ ...group, title: fixedTutoringTitles[index] ?? group.title }));
    return <section className="w-full rounded-[14px] border border-[#d8e0e8] bg-[#f8fafc] p-5"><div className="flex items-start gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-[9px] bg-[#e8f5f3]"><BookOpen className="h-4 w-4 text-[#087f75]"/></span><div><div className="flex items-center gap-2"><h2 className="text-[14px] font-semibold text-[#20394f]">Tutoría virtual</h2></div></div></div><div className="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-3">{tutoringData.map((group, index) => <TutoringCard key={`tutoring-card-${index}`} {...group}/>)}</div></section>;
}
function TutoringCard({ title, accent, rows }: TutoringGroup) {
    ...
    return <article className="overflow-hidden rounded-[10px] border border-[#dce3ea] bg-white"><div className="border-b border-[#e2e8ee] px-4 py-4"><div className="flex items-center gap-3"><span className={`flex h-8 w-8 items-center justify-center rounded-[8px] ${styles.iconBg}`}><UserRoundCheck className={`h-4 w-4 ${styles.icon}`}/></span><div><h3 className="text-[11px] font-semibold text-[#29445a]">{title}</h3></div></div></div><div className="p-3"><div className="grid grid-cols-[1fr_1fr_1fr_1fr] gap-2 px-2 py-2"><span className="text-[6px] font-semibold uppercase text-[#8296b0]">BLOQUE</span><span className="text-right text-[6px] font-semibold uppercase text-[#8296b0]">CONVOCADOS</span><span className="text-right text-[6px] font-semibold uppercase text-[#8296b0]">ASISTIERON</span><span className="text-right text-[6px] font-semibold uppercase text-[#8296b0]">% DE LOGRO</span></div><div className="space-y-1">{rows.map((row) => { const percentage = row.percentage ?? (row.invited > 0 ? Math.round((row.attended / row.invited) * 10000) / 100 : null); return <div key={row.block} className="grid min-h-[46px] grid-cols-[1fr_1fr_1fr_1fr] items-center gap-2 rounded-[8px] bg-[#f6f8fa] px-2"><span className="text-[9px] font-semibold text-[#29445a]">{row.block}</span><span className="text-right text-[8px] text-[#465d72]">{formatQuantity(row.invited)}</span><span className={`text-right text-[8px] font-medium ${styles.attended}`}>{formatQuantity(row.attended)}</span><span className="text-right text-[8px] font-semibold text-[#526176]">{percentage === null ? "—" : `${percentage}%`}</span></div>; })}</div></div></article>;
}
function AcompanamientosView({ onBack }: {
    onBack: () => void;
}) {
    const rawAcompanamientos = data.acompanamientos ?? { realizados: "0", estado: "En seguimiento" };
