...
function ModelamientosSection({ model: sourceModel, regularPercentage, remediationPercentage }: {
    model: ModelingData;
...
=======
    ...
    return <section className="w-full rounded-[14px] border border-[#d8e0e8] bg-[#f8fafc] p-5"><div className="flex items-start gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-[9px] bg-[#e8f5f3]"><BookOpen className="h-4 w-4 text-[#087f75]"/></span><div><div className="flex items-center gap-2">Al <h2 className="text-[14px] font-semibold text-[#20394f]">Tutoría virtual</h2></div><p className="mt-1 text-[8px] text-[#8da0b5]">Seguimiento de convocatoria y asistencia a sesiones de tutoría virtual</p></div></div><div className="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-3">{tutoringData.map((group, index) => <TutoringCard key={`tutoring-card-${index}`} {...group}/>)}</div></section>;
>>>>>>> Stashed changes
}
