...
function ModelamientosSection({ model, regularPercentage, remediationPercentage }: {
    ...
    const distributionCards = sortDescendingByNumber([
        ...
        { title: "Clase regular + remediación", value: model.claseRegularRemediacion.realizados, percentage: model.claseRegularRemediacion.porcentaje, color: "green" as const },
...
function TutoringCard({ title, accent, rows }: TutoringGroup) {
    ...
    return <article className="overflow-hidden rounded-[10px] border border-[#dce3ea] bg-white"><div className="border-b border-[#e2e8ee] px-4 py-4"><div className="flex items-center gap-3"><span className={`flex h-8 w-8 items-center justify-center rounded-[8px] ${styles.iconBg}`}><UserRoundCheck className={`h-4 w-4 ${styles.icon}`}/></span><div><h3 className="text-[11px] font-semibold text-[#29445a]">{title}</h3><p className="mt-1 text-[7px] text-[#8da0b5]">Asistencia por bloque</p></div></div></div><div className="p-3"><div className="grid grid-cols-[1fr_1fr_1fr] gap-2 px-2 py-2"><span className="text-[6px] font-semibold uppercase text-[#8296b0]">BLOQUE</span><span className="text-right text-[6px] font-semibold uppercase text-[#8296b0]">CONVOCADOS</span><span className="text-right text-[6px] font-semibold uppercase text-[#8296b0]">ASISTIERON</span></div><div className="space-y-1">{rows.map((row) => <div key={row.block} className="grid min-h-[46px] grid-cols-[1fr_1fr_1fr] items-center gap-2 rounded-[8px] bg-[#f6f8fa] px-2"><span className="text-[9px] font-semibold text-[#29445a]">{row.block}</span><span className="text-right text-[8px] text-[#465d72]">{row.invited}</span><span className={`text-right text-[8px] font-medium ${styles.attended}`}>{row.attended}</span></div>)}</div></div></article>;
}
