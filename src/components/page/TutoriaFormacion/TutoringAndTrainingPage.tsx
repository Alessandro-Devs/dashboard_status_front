...
function ModelingView({onBack,onNext}:{onBack:()=>void;onNext:()=>void}) {
  ...
  return <section className="mt-6"><SectionTitle index="02" title="Modelamientos" subtitle="Seguimiento de modelamientos realizados por modalidad" icon={<Monitor className="h-4 w-4"/>} accent="purple"/><div className="mt-5 grid gap-3 sm:grid-cols-2"><MetricCard title="TOTAL DOCENTES" value={model.totalDocentes} tone="blue" icon={<Users className="h-4 w-4"/>}/><MetricCard title="TOTAL REALIZADOS" value={model.meta.realizados} subtitle={`Meta: ${model.meta.total}`} badge={`${model.meta.porcentaje}%`} tone="teal" icon={<BookOpen className="h-4 w-4"/>}/></div><div className="mt-4 grid gap-3 lg:grid-cols-3"><ProgressCard title="Clase regular + remediación" totalDocentes={model.claseRegularRemediacion.totalEsperados} realizados={model.claseRegularRemediacion.realizados} porcentaje={model.claseRegularRemediacion.porcentaje}/><ProgressCard title="Solo clase regular" {...model.soloClaseRegular}/><ProgressCard title="Solo remediación" {...model.soloRemediacion}/></div><Navigation onBack={onBack} back="Accesos" onNext={onNext} next="Ver Diagnósticos"/></section>;
}
