...
function crearSankey(niveles: NivelDesempeno[], etapas: Array<{
    ...
    subeUnNivel: number;
    bajaUnNivel: number;
}>) { const same = flujo?.permaneceEnElMismoNivel ?? 80, up = flujo?.subeUnNivel ?? 10, down = flujo?.bajaUnNivel ?? 10; return { nodes: etapas.flatMap(e => niveles.map(n => ({ name: `${e.nombre} · ${n.nombre}` }))), links: etapas.slice(0, -1).flatMap((e, i) => niveles.flatMap((n, j) => { const value = e.centrosPorNivel[n.id] ?? 0, source = i * niveles.length + j, next = (i + 1) * niveles.length, links = [{ source, target: next + j, value: Math.round(value * same / 100) }]; if (j > 0)
        links.push({ source, target: next + j - 1, value: Math.round(value * up / 100) }); if (j < niveles.length - 1)
        links.push({ source, target: next + j + 1, value: Math.round(value * down / 100) }); return links.filter(l => l.value > 0); })) }; }
function Composition({ item, index }: {
    ...
    index: number;
}) { const color = index === 1 ? "#16a34a" : index === 2 ? "#dc2626" : "#64748b", Icon = index === 1 ? ArrowUpRight : index === 2 ? ArrowDownRight : Minus; return <article className="rounded-lg border bg-white p-4"><div className="flex gap-2"><Icon className="h-4 w-4" style={{ color }}/><h3 className="text-[10px] font-semibold">{item.estado}</h3></div>{tieneNumero(item.centrosEscolares) && <strong className="mt-3 block text-2xl" style={{ color }}>{item.centrosEscolares}</strong>}{tieneNumero(item.porcentajeDelTotal) && <p className="text-[8px]">{item.porcentajeDelTotal}% del total</p>}{tieneNumero(item.variacionRespectoEtapaAnterior) && <p className="mt-3 border-t pt-2 text-[7px]">Variación: {item.variacionRespectoEtapaAnterior} pp</p>}</article>; }
...
function FlowTooltip({ active, payload }: TooltipContentProps) { if (!active || !payload?.length)
    return null; const p = payload[0].payload as {
    source?: {
        name?: string;
    };
    target?: {
    ...
    value?: number;
    name?: string;
}; return <div className="rounded-md border bg-white p-2 text-[8px] shadow-lg"><strong>{p.source?.name ?? p.name}{p.target?.name ? ` → ${p.target.name}` : ""}</strong><p>{p.value ?? payload[0].value} centros escolares</p></div>; }
