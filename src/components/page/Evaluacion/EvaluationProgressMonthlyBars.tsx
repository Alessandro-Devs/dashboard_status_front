...
function numberValue(value: unknown) { const numeric = typeof value === "number" ? value : Number(String(value).replace(/,/g, "")); return Number.isFinite(numeric) ? numeric : 0; }
function formatCount(value: number) { return value.toLocaleString("es-SV"); }
function clampPercent(value: number) { return Math.min(Math.max(value, 0), 100); }
...
function loadMonthlyData(): MonthlyData[] {
  ...
  return monthOptions.map((month) => {
    ...
    return { month, blocks: rawBlocks.filter(isRecord).map((raw) => {
      ...
      const values = [1, 2, 3, 4, 5].map((level, index) => {
        const explicitPercent = numberValue(raw[`nivel${level}percent`]);
        return explicitPercent || (universe > 0 ? Math.round((data[index] / universe) * 1000) / 10 : 0);
      });
...
export default function EvaluationProgressMonthlyBars() {
  ...
  const defaultBlock = blocks.find((block) => block.toLowerCase() === "b1") ?? [...blocks].sort((a, b) => Number(a.match(/\d+/)?.[0] ?? 999) - Number(b.match(/\d+/)?.[0] ?? 999))[0] ?? "";
  const activeBlock = blocks.includes(selectedBlock) ? selectedBlock : defaultBlock;
  const activeSubject = subjects.includes(selectedSubject) ? selectedSubject : subjects.find((subject) => subject.toLowerCase() === "matematica" || subject.toLowerCase() === "matemática") ?? subjects[0] ?? "";
  ...
  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(() => { if (!blocks.includes(selectedBlock) && defaultBlock) setSelectedBlock(defaultBlock); }, [blocks, defaultBlock, selectedBlock]);
  const activeSubgroup = subgroups.includes(selectedSubgroup) ? selectedSubgroup : "";
  const availableMonthIndexes = monthlyData.reduce<number[]>((indexes, item, index) => item.blocks.length > 0 ? [...indexes, index] : indexes, []);
  const latestMonthIndex = availableMonthIndexes.at(-1) ?? -1;
  const defaultStartMonth = availableMonthIndexes.at(-2) ?? latestMonthIndex;
  const effectiveStartMonth = startMonth ?? (endMonth === null ? defaultStartMonth : null);
  const effectiveEndMonth = endMonth ?? (startMonth === null ? latestMonthIndex : null);
  ...
  const latestAverages = latestMonthIndex >= 0 ? monthlyData[latestMonthIndex].blocks : [];
  const selectedAverages = startMonth === null && endMonth === null ? latestAverages : visibleMonths.flatMap((item) => item.blocks);
...
function MonthlyBar({ block, materia, subgrupo, universe, values, data }: MonthlyBlock) { const [hoveredIndex, setHoveredIndex] = useState<number | null>(null); const hoveredValue = hoveredIndex === null ? null : values[hoveredIndex]; return <div className="rounded-[9px] border border-[#e3eaf1] bg-[#fbfcfd] p-3"><div className="mb-2 flex items-center justify-between gap-2"><h4 className="text-[11px] font-semibold text-[#334b60]">{block} · {materia}{subgrupo ? ` · ${subgrupo}` : ""}</h4><span className="text-[10px] text-[#8da0b2]">{formatCount(universe)} universo CE</span></div><div className="relative">{hoveredValue !== null && <div className="pointer-events-none absolute bottom-full right-0 z-20 mb-2 min-w-[150px] rounded-lg border border-[#d9e3ec] bg-white px-3 py-2 shadow-[0_10px_24px_rgba(30,55,80,.16)]"><div className="flex items-center gap-2 text-[10px] font-semibold text-[#334b60]"><span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: levels[hoveredIndex as number].color }} />{levels[hoveredIndex as number].label}</div><div className="mt-1.5 flex items-baseline justify-between gap-4"><strong className="text-[15px] text-[#22384c]">{formatCount(data[hoveredIndex as number])}</strong><span className="text-[10px] font-medium text-[#71869a]">{hoveredValue}%</span></div></div>}<div className="flex h-[30px] w-full overflow-hidden rounded-full bg-[#e9eef3]">{values.map((value, index) => <div key={levels[index].label} onMouseEnter={() => setHoveredIndex(index)} onMouseLeave={() => setHoveredIndex(null)} aria-label={`${levels[index].label}: ${value}%`} className="flex cursor-pointer items-center justify-center px-1 text-[8px] font-semibold text-white transition-opacity hover:opacity-85" style={{ width: `${clampPercent(value)}%`, background: levels[index].color }}>{value >= 8 ? `${value}%` : ""}</div>)}</div></div><div className="mt-2 flex justify-between text-[7px] font-medium uppercase tracking-[0.04em] text-[#667b8e] sm:text-[8px]"><span>Crítico</span><span>Excelente</span></div></div>; }
