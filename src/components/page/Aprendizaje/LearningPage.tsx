...
import { belongsToTrimester, learningLines } from "./learningData";
...
export default function LearningPage() {
  ...
  const lines=learningLines.filter(item=>(selectedLines.length===0||selectedLines.includes(item.id))&&(trimesterNumbers.length===0||trimesterNumbers.some(trimester=>belongsToTrimester(item,trimester))));
