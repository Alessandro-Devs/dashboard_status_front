...
function normalizeRowShape(item: unknown): EvolutionRow | null {
  ...
  if (ihfb === undefined && kira === undefined) return null;
...
function normalizeSeriesShape(source: Record<string, unknown>) {
  ...
  for (const platform of ["ihfb", "kira"] as const satisfies PlatformKey[]) {
    ...
    for (const point of rawPoints) {
      if (typeof point !== "object" || point === null || Array.isArray(point)) continue;

      const rowPoint = point as Record<string, unknown>;
      const day = normalizeDayLabel(
        rowPoint.day ?? rowPoint.fecha ?? rowPoint.date ?? rowPoint.dia ?? rowPoint.label,
      );
      const value = normalizeNumericValue(
        rowPoint.value ?? rowPoint.valor ?? rowPoint[platform] ?? rowPoint[platform.toUpperCase()],
      );
