...
type NoAccessHistoryItem = { fecha: string; valor: number };
type NoAccessAction = { title: string; description: string; icon: keyof typeof icons };
type NoAccessSummaryItem = { title: string; value: string; subtitle: string; color: keyof typeof teacherCardStyles; icon: keyof typeof teacherCardIcons };
...
export default function SchoolNoAccessDashboard() {
  ...
  const noAccessData = Array.isArray(noAccess.evolucion) ? noAccess.evolucion : [];
    ...
    <div className="mt-6 grid gap-5 border-t border-[#e0e7ef] pt-6 xl:grid-cols-[1.45fr_.75fr]">
      <article className="rounded-[16px] border border-[#e0e7ef] bg-white p-5"><div><h2 className="text-[17px] font-semibold">Evolución de no accesos</h2><p className="mt-1 text-[11px] text-[#8996a8]">Seguimiento diario por clases</p></div>
        <div className="mt-6 flex flex-col gap-4">
          <PlatformEvolutionChart data={noAccessData} platform="ihfb" label="IHFB" color="#4b82ee" />
          <PlatformEvolutionChart data={noAccessData} platform="kira" label="KIRA" color="#a68af4" />
        </div>
      </article>
...
function TeacherNoAccessPanel() {
  ...
  const teacherKpis = summary.map((item) => ({
    ...item,
...
type TeacherCardProps = { title: string; value: string; subtitle: string; color: string; icon: React.ElementType; iconColor: string; iconBg: string; extra?: string; dateOptions?: NoAccessHistoryItem[]; selectedDate?: string; onDateChange?: (date: string) => void };
