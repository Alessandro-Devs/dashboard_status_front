...
export default function AttendanceSection() {
  ...
  const visible = sortDescendingByNumber(
    (type === "students" ? cards.slice(0, 2) : cards.slice(2)).filter(([title]) => platforms.some((platform) => title.endsWith(platform))),
    ([, value]) => parsePercentageValue(value),
  );
