...
export default function OperationalManagementPageCurrent() {
    ...
    const formationByBlock = useMemo(() => sortDescendingByNumber((operationalData.formacion?.grupos ?? []).map(normalizeFormationGroup), (item) => item.percentage ?? 0), [operationalData.formacion?.grupos]);
