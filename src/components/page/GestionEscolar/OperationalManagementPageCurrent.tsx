...
type FormationGroup = { name: string; value?: number; total?: number; percentage: number; realizados?: number };
...
type ObservationSummary = { directoresActivos: string; observacionesRealizadas: string; retroalimentacionesRealizadas: string };
type LegacyObservationBlock = { block?: string; label?: string; activeDirectors?: number; observations?: number; observationsTarget?: number; observationsPercentage?: number; feedback?: number; feedbackTarget?: number; feedbackPercentage?: number };
...
function resolveObservationBlocks() {
  const gestionEscolar = dashboardDatabase.gestionEscolar as {
    ...
    gestionOperativa?: { observaciones?: { bloques?: LegacyObservationBlock[] } };
  };
...
function resolveObservationSummary(blocks: BlockData[]) {
  const resumen = (dashboardDatabase.gestionEscolar.gestionOperativa as { observaciones?: { resumen?: Partial<ObservationSummary> } })?.observaciones?.resumen;
  return {
...
export default function OperationalManagementPageCurrent() {
  ...
  const formationByBlock = useMemo(
    () => sortDescendingByNumber((operationalData.formacion?.grupos ?? []).map(normalizeFormationGroup), (item) => item.percentage ?? 0),
