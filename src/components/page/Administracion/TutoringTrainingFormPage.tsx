...
const withoutPercentages = (value: JsonValue): JsonValue => {
  if (Array.isArray(value)) return value.map(withoutPercentages);
  if (object(value)) return Object.fromEntries(Object.entries(value).filter(([key]) => key !== "percentage").map(([key, child]) => [key, withoutPercentages(child)]));
  return value;
};
...
const normalizeShape = (value: JsonValue): JsonValue => {
  ...
  normalized.tutoriaVirtual = fixedTutoringTitles.map((title, index) => {
    ...
    return { accent: tutoringAccents[index], rows: [], ...(object(cleanCurrent) ? cleanCurrent : {}), title };
  });
  return normalized;
};
const templateFor = (fieldKey: string, current?: JsonValue): JsonValue => {
  ...
  if (fieldKey === "rows") return { block: "", invited: 0, attended: 0 };
  return "";
};
...
function Primitive({ label, value, onChange, readOnly = false }: { label: string; value: string | number | boolean | null; onChange: (value: JsonValue) => void; readOnly?: boolean }) {
  ...
  return <label className="block text-[10px] font-semibold text-[#5d7285]">{label}<input className={`${style}${readOnly ? " cursor-not-allowed bg-[#f3f6f8] text-[#718799]" : ""}`} type={numeric ? "number" : "text"} step={numeric ? "any" : undefined} value={value ?? ""} readOnly={readOnly} onChange={(event) => onChange(numeric ? event.target.value === "" ? null : Number(event.target.value) : event.target.value)}/></label>;
}
...
function JsonField({ fieldKey, label, value, onChange, root = false, readOnlyTitle = false }: { fieldKey: string; label: string; value: JsonValue; onChange: (value: JsonValue) => void; root?: boolean; readOnlyTitle?: boolean }) {
  ...
  if (!object(value)) return <Primitive label={label} value={value} onChange={onChange} readOnly={readOnlyTitle && fieldKey === "title"}/>;

  const entries = root ? Object.entries(value) : Object.entries(value);
  const fields = <div className="grid grid-cols-[repeat(auto-fit,minmax(210px,1fr))] gap-2">{entries.map(([key, child]) => <JsonField key={key} fieldKey={key} label={humanize(key)} value={child} onChange={(updated) => onChange({ ...value, [key]: updated })} readOnlyTitle={readOnlyTitle && key === "title"}/>)}</div>;
...
export default function TutoringTrainingFormPage({ recordId }: { recordId?: number }) {
  ...
  const [data, setData] = useState<JsonValue>(() => normalizeShape(clone(dashboardDatabase.tutoriaFormacion) as JsonValue));
  ...
  useEffect(() => {
    ...
    apiFetch<{ records: Array<{ data: JsonValue }> }>("/dashboard/sections/tutoriaFormacion")
      .then(({ records }) => {
        ...
        setData(normalizeShape(emptyValues(source)));
      })
      .catch((cause) => {
