...
function ValueEditor({ label, value, onChange, depth = 0 }: { label: string; value: JsonValue; onChange: (value: JsonValue) => void; depth?: number }) {
  ...
  const content = <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-5">{orderedEntries(value).map(([key, child]) => <ValueEditor key={key} label={humanize(key)} value={child} onChange={(updated) => onChange({ ...value, [key]: updated })} depth={depth + 1}/>)}</div>;
