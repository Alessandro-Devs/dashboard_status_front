...
const campaignTemplate = { nombre: "", fecha: "", baseContactos: "", enviados: "", noEnviados: "", entregados: "", noEntregados: "", leidos: "", noLeidos: "", respuestas: "", noRespondidos: "" };
const campaignComputedFields = ["noEnviados", "noEntregados", "noLeidos", "noRespondidos"];
const campaignNumber = (value: JsonValue) => typeof value === "number" || typeof value === "string" ? Number(value) : NaN;
const normalizeCampaignList = (value: JsonValue): JsonValue[] => Array.isArray(value) ? value.map((item) => {
  if (!object(item)) return { ...campaignTemplate };
  const enviados = item.enviados ?? item.mensajesEnviados ?? "";
  const noEnviados = item.noEnviados ?? item.mensajesNoEnviados ?? "";
  const entregados = item.entregados ?? item.mensajesEntregados ?? "";
  const leidos = item.leidos ?? "";
  const respuestas = item.respuestas ?? item.respuestasRecibidas ?? "";
  const sentNumber = campaignNumber(enviados);
  const deliveredNumber = campaignNumber(entregados);
  const readNumber = campaignNumber(leidos);
  const responseNumber = campaignNumber(respuestas);
  return {
    nombre: item.nombre ?? item.name ?? "",
    fecha: item.fecha ?? item.date ?? "",
    baseContactos: item.baseContactos ?? (Number.isFinite(sentNumber) && Number.isFinite(campaignNumber(noEnviados)) ? sentNumber + campaignNumber(noEnviados) : ""),
    enviados,
    noEnviados,
    entregados,
    noEntregados: item.noEntregados ?? (Number.isFinite(sentNumber) && Number.isFinite(deliveredNumber) ? sentNumber - deliveredNumber : ""),
    leidos,
    noLeidos: item.noLeidos ?? (Number.isFinite(deliveredNumber) && Number.isFinite(readNumber) ? deliveredNumber - readNumber : ""),
    respuestas,
    noRespondidos: item.noRespondidos ?? (Number.isFinite(readNumber) && Number.isFinite(responseNumber) ? readNumber - responseNumber : ""),
  };
}) : [];
  ...
  if (object(sanitized.gestionOperativa)) {
    ...
    if (object(operational.observaciones)) {
      ...
      if (Array.isArray(observations.bloques)) {
        observations.bloques = observations.bloques.map((item) => object(item) ? reorder({ universoDocentes: "", ...item }, ["block", "universoDocentes", "activeDirectors", "activeDirectorsPercentage", "observations", "observationsTarget", "observationsPercentage", "feedback", "feedbackTarget", "feedbackPercentage"]) : item);
      }
...
const customLabels: Record<string, string> = { campanasNerds: "Campañas", noAccesosSemanal: "No accesos semanal", motivosCriticos: "Motivos", directores: "Directores", docentes: "Docentes", nombre: "Nombre", motivo: "Motivo", baseContactos: "Base de contactos", enviados: "Enviados", noEnviados: "No enviados", entregados: "Entregados", noEntregados: "No entregados", leidos: "Leídos", noLeidos: "No leídos", respuestas: "Respuestas", noRespondidos: "No respondidos", datoReal: "Dato real" };
const humanize = (key: string) => key === "realizados" ? "Participantes" : key === "universoDocentes" ? "Universo de docentes" : labels[key] ?? customLabels[key] ?? key.replace(/_/g, " ").replace(/([a-z])([A-Z0-9])/g, "$1 $2").replace(/^./, (letter) => letter.toUpperCase());
const numericFieldLabels = ["Total", "Valor", "Porcentaje", "Universo de docentes", "Porcentaje Directores activos", "Porcentaje Observaciones realizadas", "Porcentaje Retroalimentaciones realizadas", "Dato real", "Participantes", "Secciones", "Grupos", "Bloques", "Base de contactos", "Enviados", "No enviados", "Entregados", "No entregados", "Leídos", "No leídos", "Respuestas", "No respondidos", "Meta de retroalimentaciones", "Meta de observaciones", "Directores activos", "Observaciones realizadas", "Retroalimentaciones realizadas", "Centros escolares"];
const customPlaceholders: Record<string, string> = { Nombre: "Campaña Nerds 1", "Base de contactos": "568", Enviados: "530", "No enviados": "38", Entregados: "525", "No entregados": "5", Leídos: "507", "No leídos": "18", Respuestas: "16", "No respondidos": "509", Motivo: "Actividad institucional", Porcentaje: "34", "Dato real": "340" };
...
function ArrayField({ label, value, onChange, fixedLength = false, columns = false, templateValue, hiddenKeys = [] }: { label: string; value: JsonValue[]; onChange: (value: JsonValue[]) => void; fixedLength?: boolean; columns?: boolean; templateValue?: JsonValue; hiddenKeys?: string[] }) {
  const template = useRef<JsonValue>(value[0] === undefined ? templateValue ?? "" : clone(value[0]));
  ...
  return <div className="col-span-full rounded-lg border border-[#dce7ef] bg-[#f8fbfe] p-2.5"><div className="mb-2 flex items-center justify-between"><div><p className="text-[11px] font-bold text-[#294b68]">{label}</p><p className="text-[9px] text-[#8a9cab]">{value.length} registros{fixedLength ? " · estructura fija" : ""}</p></div>{!fixedLength && <button type="button" onClick={add} className="inline-flex items-center gap-1 rounded-md border border-[#bfd8eb] bg-white px-2 py-1 text-[10px] font-semibold text-[#176fc8]"><Plus size={11}/>Agregar fila</button>}</div>{value.length === 0 ? fixedLength ? <p className="rounded-md border border-dashed border-[#bcd3e4] bg-white py-4 text-center text-[10px] text-[#6f8ca2]">No hay días configurados.</p> : <button type="button" onClick={add} className="w-full rounded-md border border-dashed border-[#bcd3e4] bg-white py-4 text-center text-[10px] text-[#6f8ca2]">+ Agregar el primer registro</button> : <div className={columns ? "grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-3" : "space-y-2"}>{value.map((item, index) => <div key={index} className="relative rounded-lg border border-[#dce7ef] bg-white p-2.5 pt-7"><span className="absolute left-2.5 top-2 text-[9px] font-bold text-[#688196]">{itemLabel} {index + 1}</span>{!fixedLength && <button type="button" onClick={() => onChange(value.filter((_, itemIndex) => itemIndex !== index))} className="absolute right-2 top-1.5 rounded p-1 text-[#c85a5a] hover:bg-red-50" aria-label={`Eliminar fila ${index + 1}`}><Trash2 size={12}/></button>}<JsonField label={`${label} ${index + 1}`} value={item} hiddenKeys={hiddenKeys} onChange={(updated) => onChange(value.map((current, itemIndex) => itemIndex === index ? updated : current))} root={object(item)}/></div>)}</div>}</div>;
}
...
function CampaignsField({ value, onChange }: { value: JsonValue; onChange: (value: JsonValue) => void }) {
  ...
  const update = (group: "directores" | "docentes", records: JsonValue[]) => onChange({ ...campaigns, [group]: normalizeCampaignList(records) });
  return <details className="group col-span-full rounded-lg border border-[#dce7ef] bg-[#f8fbfe]"><summary className="flex cursor-pointer list-none items-center justify-between rounded-lg bg-[#f5f9fc] px-3 py-2 text-[11px] font-bold text-[#294b68]">Campañas<ChevronDown size={13} className="transition group-open:rotate-180"/></summary><div className="border-t border-[#e6edf2] p-2"><p className="mb-3 text-[9px] text-[#8a9cab]">Registra nombre, fecha y los datos principales de cada campaña.</p><div className="grid grid-cols-1 gap-3"><details className="w-full rounded-lg border border-[#dce7ef] bg-white"><summary className="cursor-pointer list-none px-3 py-2 text-[10px] font-bold uppercase tracking-wide text-[#526b80]">Directores</summary><div className="border-t border-[#e6edf2] p-2"><ArrayField label="Directores" value={Array.isArray(campaigns.directores) ? campaigns.directores : []} templateValue={campaignTemplate} hiddenKeys={campaignComputedFields} onChange={(updated) => update("directores", updated)}/></div></details><details className="w-full rounded-lg border border-[#dce7ef] bg-white"><summary className="cursor-pointer list-none px-3 py-2 text-[10px] font-bold uppercase tracking-wide text-[#526b80]">Docentes</summary><div className="border-t border-[#e6edf2] p-2"><ArrayField label="Docentes" value={Array.isArray(campaigns.docentes) ? campaigns.docentes : []} templateValue={campaignTemplate} hiddenKeys={campaignComputedFields} onChange={(updated) => update("docentes", updated)}/></div></details></div></div></details>;
}

function JsonField({ label, value, onChange, root = false, fixedLength = false, placeholder, hiddenKeys = [] }: { label: string; value: JsonValue; onChange: (value: JsonValue) => void; root?: boolean; fixedLength?: boolean; placeholder?: string; hiddenKeys?: string[] }) {
  if (Array.isArray(value)) {
    const arrayField = <ArrayField label={label} value={value} onChange={onChange} fixedLength={fixedLength} hiddenKeys={hiddenKeys}/>;
  ...
  const fields = label === "Formación" ? <div className="grid grid-cols-[repeat(auto-fit,minmax(210px,1fr))] gap-2"><div className="col-span-full rounded-lg border border-[#d9e5ee] bg-white shadow-[0_2px_8px_rgba(27,58,87,.04)]"><div className="rounded-t-lg bg-[#f5f9fc] px-3 py-2 text-[11px] font-bold text-[#294b68]">Resumen</div><div className="grid grid-cols-[repeat(auto-fit,minmax(210px,1fr))] gap-2 border-t border-[#e6edf2] p-3">{["secciones", "participantes"].filter((key) => key in value && !hiddenKeys.includes(key)).map((key) => field([key, value[key]]))}</div></div>{Object.entries(value).filter(([key]) => !["secciones", "participantes"].includes(key) && !hiddenKeys.includes(key)).map(field)}</div> : <div className="grid grid-cols-[repeat(auto-fit,minmax(210px,1fr))] gap-2">{Object.entries(value).filter(([key]) => !hiddenKeys.includes(key)).map(field)}</div>;
