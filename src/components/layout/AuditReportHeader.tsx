...
import { getDefaultDashboardSectionLabel } from "@/lib/dashboardSections";
...
export default function AuditReportHeader() {
  ...
  const { availableSections } = useDashboardData();
  const [, setNavigating] = useState(false);
  const pathname = usePathname();
  const router = useRouter();
  const hydrated = useSyncExternalStore(subscribeToHydration, () => true, () => false);
  const navItems = availableSections.map((section) => section.label);
  ...
  const rawSection = !hydrated
    ...
    : pathname.startsWith("/gestion-escolar")
      ? "Gestión Escolar"
      : state.activeSection;
  const section = navItems.find((item) => item === rawSection) ?? fallbackSection;
