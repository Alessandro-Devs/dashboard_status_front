...
function MultiSelect({ label,header,options,selected,setSelected,width }: { label:string;header:string;options:Option[];selected:string[];setSelected:(value:string[])=>void;width:string }) {
  const [open,setOpen] = useState(false); const ref=useRef<HTMLDivElement>(null); const all=selected.length===0;
